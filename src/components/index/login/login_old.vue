<template>
  <div class="login">
    ...
    <form class="" onsubmit="return false">
      ...
      <div class="login_Ver">
        <input type="text" class="Verification" placeholder="输入获取短息验证码" v-model="inputPhoneCode">
      </div>

      <div class="login_Ver">
        <input type="text" class="Verification" placeholder="密码">
      </div>
      <div class="login_Ver">
...
//  			要在图文验证码正确的前提下,触发我这个方法
					if(!this.flag){
//				如果我图文验证码是错误的就退出这个给手机发送短信的方法
						return;
					}
					//发送axios请求给手机发短信
		  		console.log("点击获取手机验证码的方法");
		  		var vdcode = parseInt(Math.random() * (9999 - 1000 + 1) + 1000);
	        localStorage.setItem("phoneCode",vdcode);
  			})	
  	},
  	//点击切换图片验证码的方法
		changePicCode(){
